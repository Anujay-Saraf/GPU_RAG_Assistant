...
# -------------------------------------------------------------
# Real-Time API Key Live Connection Prober
# -------------------------------------------------------------
def validate_api_key(provider: str, key: str) -> tuple[bool, str]:
    """Tests if an API key is active and authenticated against the selected provider."""
    if not key or len(key.strip()) < 8:
        return False, "Key format too short or empty."
    ...
    try:
        if provider == "openrouter":
            ...
            with urllib.request.urlopen(req, timeout=3.5) as resp:
                if resp.status == 200:
                    ...
                    return True, f"Verified ({label})"
                return False, f"Server rejected key (Code {resp.status})"

        elif provider == "gemini":
            genai.configure(api_key=key)
            # Lightweight models check
            for _ in genai.list_models():
                return True, "Verified (Google Cloud)"
            return True, "Verified"

        elif provider == "openai":
            client = openai.OpenAI(api_key=key, timeout=3.5)
            client.models.list()
            return True, "Verified (OpenAI API)"
        ...
        if "401" in err_msg or "auth" in err_msg.lower():
            ...
            return False, "Key valid but quota exceeded (429)"
        return False, f"Connection check failed: {err_msg[:60]}"
        ...
        with urllib.request.urlopen(req, timeout=4.0) as resp:
            if resp.status == 200:
                ...
                for m in models:
                    ...
                    is_zero = (
                        str(pricing.get("prompt", "")).strip() in ["0", "0.0"] and
                        str(pricing.get("completion", "")).strip() in ["0", "0.0"]
                    )
                    if model_id.endswith(":free") or is_zero:
                        if not any(bad in model_id.lower() for bad in ["rerank", "embed", "guard", "moderation"]):
...
class FastEmbedding(EmbeddingFunction[Documents]):
    ...
    def __call__(self, input: Documents) -> Embeddings:
        return self.model.encode(input, batch_size=64, normalize_embeddings=True, show_progress_bar=False).tolist()

@st.cache_resource(show_spinner="Loading Neural Embedding & Re-Ranking Engines...")
    ...
    for m in all_meta:
        ...
        if src:
            if src not in doc_summary:
                ...
                doc_summary[src]["pages"] = max(doc_summary[src]["pages"], int(m.get("page", 1)))
...
def ingest_pdf_files(uploaded_files: list, uploader_role: str = "user") -> tuple[int, list]:
    ...
    for f in uploaded_files:
        ...
        if f.name in current_inventory and current_inventory[f.name]["file_size"] == file_size:
        ...
        if all_chunks:
            ...
            for i in range(0, len(all_chunks), batch_size):
                ...
                collection.upsert(
                    ...
                    ids=[f"{f.name}_{uuid.uuid4().hex[:8]}_{j}" for j in range(i, i + len(batch))]
                )
...
# -------------------------------------------------------------
# Dynamic Credential Resolver
# -------------------------------------------------------------
def resolve_effective_provider_and_key() -> tuple[str, str, str]:
    """
    Returns: (provider_name, valid_api_key, status_description)
    """
    # 1. User BYOK Session Override
    user_prov = st.session_state.get("user_selected_provider")
    user_key = st.session_state.get("user_custom_api_key", "").strip()

    if user_prov and user_key:
        is_valid, reason = validate_api_key(user_prov, user_key)
        if is_valid:
            return user_prov, user_key, f"User Session ({reason})"
        else:
            return user_prov, "", f"Invalid User Key: {reason}"
    ...
    if admin_prov:
        ...
        if admin_key:
            return admin_prov, admin_key, "Admin Key"

    # 3. System Defaults (Streamlit Secrets / Env)
    sys_prov = admin_prov or get_secret("ACTIVE_PROVIDER", "openrouter")
    sys_key = ""
    if sys_prov == "openrouter":
        sys_key = get_secret("OPENROUTER_API_KEY")
    elif sys_prov == "gemini":
        sys_key = get_secret("GEMINI_API_KEY")
    elif sys_prov == "openai":
        sys_key = get_secret("OPENAI_API_KEY")

    if sys_key:
        return sys_prov, sys_key, "System Secret"

    return sys_prov, "", "Extractive Mode (No Key)"

# -------------------------------------------------------------
# Conversational Context & Intent Engine
# -------------------------------------------------------------
ACRONYM_MAP = {
...
def expand_query(query: str) -> list:
    ...
    if any(w in query.lower() for w in [" and ", " vs ", " difference ", " compare "]):
...
def contextualize_conversation_intent(history: list, latest_query: str, provider: str, api_key: str) -> str:
    if not history or len(history) < 2 or not api_key:
        return latest_query

    intent_triggers = [" it", " this", " that", " them", " previous", " above", " implement it", " code for it",
                       "optimize", "improve", "explain more", "why", "how about", "what about", "instead", 
                       "second one", "first one", "in python", "in cpp", "in c++", "in java"]
    needs_context = any(t in latest_query.lower() for t in intent_triggers) or len(latest_query.split()) <= 4
    if not needs_context:
        return latest_query

    recent_turns = [f"{m['role'].upper()}: {m['content'][:300].replace(chr(10), ' ')}" for m in history[-6:]]
    prompt = (
        "Given this ongoing technical discussion, rewrite the user's latest follow-up into an explicit, "
        "self-contained search query that captures the full contextual intent and specific topics. "
    ...
    try:
        if provider == "gemini":
            ...
            models = ["gpt-4o-mini"] if provider == "openai" else get_dynamic_free_models()
...
def hybrid_search(sub_queries: list, selected_sources: list = None, top_k_per_query: int = 8) -> list:
    bm25_data = st.session_state.get("bm25_cache", {"bm25": None, "docs": [], "metas": []})
    ...
    for sq in sub_queries:
        try:
            ...
            for rank, (doc, meta, dist) in enumerate(zip(d_docs, d_metas, d_dists)):
                ...
                if key not in candidates:
                    candidates[key] = {"text": doc, "meta": meta, "cosine_sim": max(0.0, 1.0 - dist), "rrf": score}
                else:
        ...
        if bm25_data["bm25"]:
            ...
            for rank, idx in enumerate(scores.argsort()[::-1][:top_k_per_query]):
                ...
                score = 1.0 / (60.0 + rank + 1)
                if key not in candidates:
                    candidates[key] = {"text": doc, "meta": meta, "cosine_sim": 0.50, "rrf": score}
                else:
                    candidates[key]["rrf"] += score

    sorted_cands = sorted(candidates.values(), key=lambda x: x["rrf"], reverse=True)
    return sorted_cands[:20]

def stream_llm(provider: str, api_key: str, system_prompt: str, user_prompt: str, conversation_history: list = None):
    if not api_key:
    ...
    if provider == "gemini":
        ...
        for candidate_model in live_free_models:
            try:
                ...
                logger.warning(f"OpenRouter candidate [{candidate_model}] failed: {model_err}")
                continue

        if not streamed:
            raise RuntimeError("All free models are currently unavailable on OpenRouter.")
...
def render_user_page():
    ...
    # Sidebar: BYOK & File Management
    with st.sidebar:
        with st.expander("🔑 BYOK: Custom API Key (Optional)", expanded=False):
            st.caption("Supply your personal API key for this session:")
            user_provider_choice = st.selectbox(
                "Select Provider",
                ["openrouter", "gemini", "openai"],
                index=0,
                key="user_prov_select",
                format_func=lambda x: "OpenRouter (Auto Free)" if x == "openrouter" else ("Google Gemini" if x == "gemini" else "OpenAI")
            )
            ...
            with col_save:
                if st.button("Apply & Test Key", use_container_width=True):
                    if user_key_input:
                        with st.spinner("Verifying credentials..."):
                            is_ok, reason = validate_api_key(user_provider_choice, user_key_input)
                            if is_ok:
                                st.session_state.user_custom_api_key = user_key_input.strip()
                                st.success(f"✅ {reason}")
                                time.sleep(0.5)
                                st.rerun()
                            else:
        ...
        if active_key:
            st.success(f"🟢 **Engine:** {active_prov.upper()}\n\n*{key_src}*")
        else:
            st.warning(f"🟡 **Engine:** EXTRACTIVE FALLBACK\n\n*{key_src}*")

        st.markdown("---")
        st.subheader("📤 Upload Documents")
        user_uploaded_files = st.file_uploader("Upload PDF Files", type=["pdf"], accept_multiple_files=True, key="user_file_uploader")
        
        if st.button("⚡ Index My Documents", use_container_width=True, key="user_index_btn") and user_uploaded_files:
            with st.status("Ingesting & Indexing PDF(s)...", expanded=True) as box:
                count, skipped = ingest_pdf_files(user_uploaded_files, uploader_role="user")
                box.update(label=f"Done! ({count} chunks indexed, {len(skipped)} duplicates bypassed)", state="complete")
        ...
        if current_inventory:
            ...
            for name, info in current_inventory.items():
                size_kb = round(info['file_size'] / 1024, 1) if info['file_size'] > 0 else "N/A"
                st.caption(f"📄 **{name}** (~{info['pages']} pgs | {info['chunks']} chunks | {size_kb} KB)")
        else:
            selected_docs = []
    ...
    with header_col2:
        if st.session_state.user_messages:
            st.markdown(
                '<div style="text-align: right; margin-top: 1.5rem;"><a href="#latest_chat_anchor" class="jump-to-latest">⬇️ Jump to Latest</a></div>',
                unsafe_allow_html=True
            )

    # -------------------------------------------------------------
    # Chat Stream with In-Row Split Citations
    # -------------------------------------------------------------
            ...
            with st.chat_message("assistant"):
                ...
                with col_ans:
                    if msg.get("mode") == "general_knowledge":
                        st.info("💡 **Answered from General AI Knowledge** *(Topic not found in selected documents)*")
                    elif msg.get("mode") == "extractive_fallback":
                        st.warning("ℹ️ **Extractive Fallback Mode:** Direct verified passages from documents.")
                    st.markdown(msg["content"])

                with col_cite:
                    if msg.get("metrics_md"):
                        with st.expander("📚 Citations & Evidence", expanded=msg.get("open_citations", False)):
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            catalog_patterns = [r"which (all )?(notes|docs|documents|files|pdfs)", r"what (notes|docs|files) (do we have|exist)", r"list (all )?(the )?notes"]
            if any(re.search(p, prompt.lower()) for p in catalog_patterns):
                inv = get_indexed_inventory()
                if not inv:
                    summary = "The knowledge base is currently empty. Please upload PDF files in the sidebar."
                else:
                    lines = ["Here is the inventory of indexed documents:\n"]
                    for idx, (doc_name, dinfo) in enumerate(inv.items(), 1):
                        lines.append(f"- **[{idx}] `{doc_name}`** (~{dinfo['pages']} pages, {dinfo['chunks']} chunks indexed)")
                    summary = "\n".join(lines)
            ...
            status_box.update(label="🧠 Resolving conversational context & sub-queries...", state="running")
            ...
            if collection.count() == 0:
                ...
                resp_ph.error("No documents have been indexed yet. Please upload PDF files in the sidebar.")
                st.stop()

            # 3. Hybrid Search & Neural Re-Ranking
            status_box.update(label="🔍 Hybrid search & Cross-Encoder ranking...", state="running")
            t0 = time.time()
            ...
            sigmoid_probs = (1.0 / (1.0 + np.exp(-raw_logits))) if len(raw_logits) > 0 else np.array([])
            rr_time = time.time() - t1

            max_confidence = float(np.max(sigmoid_probs)) if len(sigmoid_probs) > 0 else 0.0
            is_code = any(t in prompt.lower() for t in ["code", "implement", "python", "script", "function", "class"])

            # 4. Generation with Multi-Turn Memory & Extractive Fallback
            t_gen = time.time()
            generation_succeeded = False

            if active_key:
                status_box.update(label=f"✍️ Streaming verified response via {active_prov.upper()}...", state="running")
                try:
                    if max_confidence < 0.15:
                        final_mode = "general_knowledge"
                        notice_ph.info("💡 **Answered from General AI Knowledge** *(Topic not found in selected documents)*")
                        user_prompt = f"Topic/Task: {standalone_query}\n\nTechnical Explanation & Code:"
                        for chunk in stream_llm(active_prov, active_key, GENERAL_FALLBACK_SYSTEM_PROMPT, user_prompt, conversation_history=st.session_state.user_messages[:-1]):
                            full_response += chunk
                        ...
                        user_prompt = f"Context Sources:\n{context_str}\n\nTask: Provide clean, working Python code for '{standalone_query}' based on context.\nInclude inline citations [1], [2].\n\nAnswer:" if is_code else f"Context Sources:\n{context_str}\n\nQuestion: {standalone_query}\n\nCited Answer:"

                        for chunk in stream_llm(active_prov, active_key, GROUNDED_SYSTEM_PROMPT, user_prompt, conversation_history=st.session_state.user_messages[:-1]):
                            full_response += chunk
                            resp_ph.markdown(full_response + "▌")
                    ...
                    logger.warning(f"LLM generation failed ({gen_err}). Triggering Extractive Fallback...")
                    full_response = ""

            # PATH: Extractive Fallback (No valid key or LLM error)
            if not generation_succeeded:
                final_mode = "extractive_fallback"
                status_box.update(label="⚡ Extractive Mode (Local Neural Engine)", state="complete")
                
                if active_key:
                    notice_ph.warning("⚠️ **LLM Connection Alert:** Cloud generation encountered an issue. Displaying direct verified passages from documents.")
                else:
                    notice_ph.info("ℹ️ **Extractive Mode:** Direct top-ranked passages verified by local neural cross-encoder.")
                ...
                if max_confidence >= 0.15:
                    ...
                    for rank, i in enumerate(ranked_idx, 1):
                        ...
                        lines.append(f"**[{rank}] {src} (Page {pg})** — *{conf}% relevance*\n> " + c['text'].strip().replace("\n", "\n> ") + "\n")
                    
                    full_response = "\n".join(lines)
                    resp_ph.markdown(full_response)
                else:
                    full_response = "No closely matching sections found in the selected documents."
...
*Note: No document section met the grounding threshold.*"""
            ...
            # Render citation in the right sub-column for this turn
            with col_cite:
...
def render_admin_page():
    ...
    if not st.session_state.admin_logged_in:
        ...
        entered_pass = st.text_input("Enter Master Passkey", type="password", key="admin_auth_key")
    ...
    tab1, tab2 = st.tabs(["⚙️ Provider & Credentials", "📤 Document Ingestion & Database"])
    ...
    with tab1:
        ...
        cur_prov = st.session_state.get("admin_active_provider", get_secret("ACTIVE_PROVIDER", "openrouter"))
    ...
    with tab2:
        ...
        admin_files = st.file_uploader("Upload PDF Files", type=["pdf"], accept_multiple_files=True, key="admin_file_uploader")
        ...
        if st.button("Process & Index", use_container_width=True) and admin_files:
            with st.status("Indexing documents...", expanded=True) as status_box:
                ...
                status_box.update(
                    label=f"Completed: {count} new chunks indexed, {len(skipped)} duplicate files skipped.",
                    state="complete"
                )
                st.rerun()
        ...
        if inv:
            for name, info in inv.items():
                size_kb = round(info['file_size'] / 1024, 1) if info['file_size'] > 0 else "N/A"
                st.write(f"- **`{name}`** (~{info['pages']} pages | {info['chunks']} chunks | {size_kb} KB | Tag: `{info.get('uploader', 'system')}`)")
...
# =============================================================
user_page = st.Page(render_user_page, title="User Portal", icon="💬", url_path="user", default=True)
admin_page = st.Page(render_admin_page, title="Admin Console", icon="🔐", url_path="admin")
