...
def ingest_pdf_files(uploaded_files: list, uploader_role: str = "user") -> tuple[int, list]:
    ...
    for f in uploaded_files:
        ...
        # Duplicate detection (exact name + size)
...
def resolve_effective_provider_and_key() -> tuple[str, str, str]:
    ...
    1. User Session Override (Highest priority)
    2. Admin Session Override
    3. Streamlit Cloud Secrets / Environment Variables (Default fallback)
    
    Returns: (provider_name, api_key, source_label)
    """
    # 1. Check if user configured a session key
    ...
    if user_override_prov and user_key:
        return user_override_prov, user_key, "User Session Override"

    # 2. Check Admin session configuration
    ...
    # 3. Check System Defaults (Secrets/Env)
    ...
    # 4. No Key Available anywhere
    return system_prov, "", "No Credentials (Extractive Fallback)"
...
def contextualize_pronouns(history: list, latest_query: str, provider: str, api_key: str) -> str:
    if not history or len(history) < 2 or not api_key:
        return latest_query
    ambiguous = [" it", " this", " that", " them", " previous", " above", " implement it", " code for it"]
    if not any(t in latest_query.lower() for t in ambiguous):
        return latest_query

    history_str = "\n".join([f"{m['role'].upper()}: {m['content'][:250]}" for m in history[-4:]])
    ...
    try:
        if provider == "gemini":
            ...
            available_models = ["gpt-4o-mini"] if provider == "openai" else get_dynamic_free_models()
...
def stream_llm(provider: str, api_key: str, system_prompt: str, user_prompt: str):
    if not api_key:
...
def render_user_page():
    ...
    st.caption("Real-Time RAG | Multi-Document Ingestion | Seamless Extractive Fallback")
    ...
    with st.sidebar:
        ...
        # Display active credential badge
        active_prov, active_key, key_src = resolve_effective_provider_and_key()
        if active_key:
            st.success(f"🟢 Active Engine: **{active_prov.upper()}** ({key_src})")
        else:
    ...
    for msg in st.session_state.user_messages:
        with st.chat_message(msg["role"]):
            if msg.get("mode") == "general_knowledge":
                ...
                st.warning("ℹ️ **Extractive Fallback Mode:** Direct verified passages from your documents.")
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            if any(re.search(p, prompt.lower()) for p in catalog_patterns):
                ...
                st.session_state.user_messages.append({"role": "assistant", "content": summary, "mode": "catalog"})
                st.stop()

            # 3. Contextualization & Sub-Queries
            status_box.update(label="🧠 Expanding query intent...", state="running")
            standalone_query = contextualize_pronouns(st.session_state.user_messages[:-1], prompt, active_prov, active_key)
            ...
            # 5. Adaptive Generation with Absolute Extractive Fallback Guarantee
            t_gen = time.time()
            ...
            if active_key:
                ...
                try:
                    if max_confidence < 0.15:
                        ...
                        for chunk in stream_llm(active_prov, active_key, GENERAL_FALLBACK_SYSTEM_PROMPT, user_prompt):
                            full_response += chunk
                        ...
                        for chunk in stream_llm(active_prov, active_key, GROUNDED_SYSTEM_PROMPT, user_prompt):
                            full_response += chunk
                            resp_ph.markdown(full_response + "▌")
            ...
            # PATH: Extractive Fallback (Triggered if no key OR if LLM call fails)
            if not generation_succeeded:
                final_mode = "extractive_fallback"
                status_box.update(label="⚡ Extractive Mode (Local Neural Engine)", state="complete")
                
                if active_key:
                    notice_ph.warning(f"⚠️ **LLM Connection Alert:** Cloud generation encountered an issue. Falling back to verified excerpts from your documents below.")
                else:
            ...
            # 6. Build Verification & Grounding Audit Expander
            if max_confidence >= 0.15:
                ranked_idx = [i for i in sigmoid_probs.argsort()[::-1] if sigmoid_probs[i] >= 0.15][:4]
                final_cands = [candidates[i] for i in ranked_idx]
                sources_payload = [{
...
"""
                with st.expander("📚 Evidence & Verification Audit", expanded=(final_mode == "grounded_rag")):
